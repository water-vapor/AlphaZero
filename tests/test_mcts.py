...
class TestMCTS(unittest.TestCase):
	...
	def setUp(self):
		self.gs = GameState()
		self.mcts = MCTSearch(random_state_transform, policy_value_generator, n_playout=2)
	...
	def test_playout_with_pass(self):
		...
		def stop_early_policy(state):
			if len(state.get_history()) <= 4:
				...
				return []
		self.mcts = MCTS(zero_value, stop_early_policy, stop_early_policy, n_playout=2)
		self.mcts._playout(self.gs.copy(), 8)
