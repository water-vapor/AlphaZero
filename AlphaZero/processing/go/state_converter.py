...
class TensorActionConverter(object):
    ...
    def tensor_to_action(self, tensor):
        ...
        Args:
            tensor: a 1D prob tensor with length 362, last element is PASS move

        Returns:
            a list of (action, prob)
        """
        res = [((i // 19, i % 19), tensor[i]) for i in range(361)]
        res.append((go.PASS_MOVE, tensor[361]))
...
def lr_reflection(action_prob):
    """ Flips the coordinate of action probability vector like np.fliplr
        Note that go.PASS_MOVE should be placed at the end of this vector
    ...
    """
    for i in range(361):
        action_prob[i] = ((action_prob[i][0][0], 18 - action_prob[i][0][1]), action_prob[i][1])
...
def reverse_nprot90(action_prob, transform_id):
    ...
    if transform_id == 0:
        ...
        for i in range(361):
            action_prob[i] = ((action_prob[i][0][1], 18 - action_prob[i][0][0]), action_prob[i][1])
        return
    elif transform_id == 2:
        for i in range(361):
            action_prob[i] = ((18 - action_prob[i][0][0], 18 - action_prob[i][0][1]), action_prob[i][1])
        return
    elif transform_id == 3:
        for i in range(361):
            action_prob[i] = ((18 - action_prob[i][0][1], action_prob[i][0][0]), action_prob[i][1])
        return
    else:
