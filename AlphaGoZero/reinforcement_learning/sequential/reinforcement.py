...
def main():
	""" File structure: The program uses 'data/models', 'data/selfplay/' and 'data/evaluations' as defaults.
		...
		data/evaluations cntains evaluation matches, whose names are concatenated from the player's name
	...
	if os.path.exists(os.path.join(args.directory, 'models')) and os.path.exists(os.path.join(args.directory, 'selfplay')):
		# Check configuration file
		assert os.path.isfile(os.path.join(args.directory, 'config.json')), 'Files found but config is missing, please consider add one manaully'
	...
	for t in range(args.iteration):
		...
		new_model_name = optimize(rl['selfplay'][-25:], rl_info['models'][-1], base_dir=args.directory)
